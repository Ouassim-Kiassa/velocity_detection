...
class WaterVelocityGUI:
    def __init__(self, master):
        ...
        self.known_distance_var = tk.StringVar(value="100.0") # Default 100cm = 1m
    ...
    def select_video_for_setup(self):
        ...
        if video_path:
            ...
            self.selected_setup_video_label.config(text=f"Setup Video: {os.path.basename(video_path)}")
    ...
    def calibrate_mpp_on_frame(self):
        ...
        self.master.withdraw() # Hide main Tkinter window before showing OpenCV window

        cv2.namedWindow(self.mpp_calib_window_name, cv2.WINDOW_AUTOSIZE)
        ...
        cv2.waitKey(1) # Crucial for some systems to render and focus
        ...
        while True:
            ...
            if key == 13: # Enter
                if len(self.mpp_calib_points) == 2:
                    ...
                    self._log_message_gui("Please click 2 points before pressing Enter for MPP calibration.")
        ...
        if calib_confirmed and len(self.mpp_calib_points) == 2:
            ...
            if pixel_distance > 0:
                known_distance_m = known_distance_cm / 100.0 # Convert cm to m
                ...
                self._log_message_gui("MPP calibration failed: Pixel distance is zero.")
        else:
            self._log_message_gui("MPP calibration cancelled or incomplete.")
    ...
    def draw_roi_on_frame(self):
        ...
        cv2.namedWindow(self.roi_drawing_window_name, cv2.WINDOW_AUTOSIZE)
        ...
        while True:
            ...
            if key == 13: # Enter
                if len(self.roi_drawing_points) == 4:
                    ...
                    self._log_message_gui("Please click 4 points before pressing Enter for ROI.")
            elif key == ord('r') or key == ord('R'):
                self.roi_drawing_points = []
                self.roi_drawing_frame_copy = frame_for_roi.copy()
                cv2.imshow(self.roi_drawing_window_name, self.roi_drawing_frame_copy)
