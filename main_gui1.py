...
def estimate_water_velocity(video_path, meters_per_pixel, roi=None, use_dense_flow=False, skip_frames=0, max_frames_to_process=None, log_callback=None):
    ...
    Args:
        ...
        roi (tuple or list, optional): 
            - None: Full frame.
            - Tuple (x, y, w, h): Rectangular ROI.
            - List of 4 points [(x1,y1), (x2,y2), (x3,y3), (x4,y4)]: Quadrilateral ROI.
            Defaults to None.
    ...
    is_quad_roi = False
    roi_points_relative_to_crop = None # For quad ROI, points relative to its bounding box
    x_b, y_b, w_b, h_b = 0, 0, fw_orig, fh_orig # Bounding box for processing, defaults to full frame
    ...
    if roi:
        ...
        elif isinstance(roi, tuple) and len(roi) == 4: # Rectangular (x,y,w,h)
    ...
    while True:
        ...
        if roi: # If any ROI was defined and valid, use its bounding box
        ...
        if use_dense_flow:
            ...
            else: # Rectangular ROI or full frame
            ...
            if p1 is not None and st is not None:
                ...
                if is_quad_roi and roi_points_relative_to_crop is not None and len(good_old_crop) > 0:
                    ...
                    for i, pt_old_crop_coords in enumerate(good_old_crop):
                        # cv2.pointPolygonTest expects tuple for point
...
class WaterVelocityGUI:
    def __init__(self, master):
        ...
        self.roi_var = tk.StringVar() # Will store "x1,y1,x2,y2,x3,y3,x4,y4"
    ...
    def setup_ui(self):
        ...
        roi_outer_frame = ttk.Frame(input_frame)
        roi_outer_frame.grid(row=row_idx, column=0, columnspan=3, sticky="ew", pady=2)
        roi_outer_frame.columnconfigure(1, weight=1)

        ttk.Label(roi_outer_frame, text="ROI (4-points):").grid(row=0, column=0, sticky="w", pady=(0,2))
        ...
        self.draw_roi_button = ttk.Button(roi_outer_frame, text="Draw ROI (4 Clicks)", command=self.draw_roi_on_frame, state=tk.DISABLED)
    ...
    def _mouse_callback_roi(self, event, x, y, flags, param):
        # param is (window_name, original_frame_for_reset)
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if len(self.roi_drawing_points) < 4:
                ...
                if len(self.roi_drawing_points) == 4:
                    cv2.line(self.roi_drawing_frame_copy, self.roi_drawing_points[3], self.roi_drawing_points[0], (0, 0, 255), 2) # Close polygon
    ...
    def draw_roi_on_frame(self):
        ...
        self.roi_drawing_points = [] # Reset points for this session
        self.roi_drawing_frame_copy = frame_for_roi.copy() # Work on a copy

        cv2.namedWindow(self.roi_drawing_window_name, cv2.WINDOW_AUTOSIZE) # or WINDOW_NORMAL
        ...
        self._log_message_gui(f"ROI selection window '{self.roi_drawing_window_name}' opened. Click 4 points. \nENTER to confirm, R to reset, ESC/C to cancel.")
        ...
        while True:
            ...
            if key == 13: # Enter
                if len(self.roi_drawing_points) == 4:
                    ...
                    self._log_message_gui("Please click 4 points before pressing Enter.") # Log to GUI too
                    # cv2.displayOverlay(self.roi_drawing_window_name, "Click 4 points first!", 1000) # If OpenCV version supports it
            elif key == ord('r') or key == ord('R'): # Reset
                self.roi_drawing_points = []
                self.roi_drawing_frame_copy = frame_for_roi.copy()
                cv2.imshow(self.roi_drawing_window_name, self.roi_drawing_frame_copy)
                self._log_message_gui("ROI points reset. Click 4 new points.")
            elif key == 27 or key == ord('c') or key == ord('C'): # ESC or C
                break
        ...
        if roi_confirmed and len(self.roi_drawing_points) == 4:
            # Flatten points list: [(x1,y1), (x2,y2)...] -> [x1,y1,x2,y2...]
            ...
            self.roi_var.set("") # Clear if cancelled or not confirmed with 4 points
            self._log_message_gui("ROI selection cancelled or incomplete.")
        ...
        try:
            ...
            if roi_str:
                try:
                    ...
                    if len(coords) == 8: # Quadrilateral: x1,y1,x2,y2,x3,y3,x4,y4
                        parsed_roi = [(coords[i], coords[i+1]) for i in range(0, 8, 2)]
                        # Basic validation for non-negative coordinates, though `estimate_water_velocity` does more.
                        if any(val < 0 for val in coords):
                            raise ValueError("ROI coordinates must be non-negative.")
                    else:
                        raise ValueError("ROI string must contain 8 comma-separated integers for 4 points.")
                except ValueError as e:
                    self.log_message_thread_safe(f"Error: Invalid ROI string '{roi_str}'. {e}")
                    return
            ...
            for ext in extensions:
                ...
                video_files.extend(glob.glob(os.path.join(folder_path, f"*.{ext.upper()}"))) # Case-insensitive
            video_files = sorted(list(set(video_files))) # Unique and sorted
